...
locals {
  ...
  flow_log_group_arns = [
    ...
    "arn:${data.aws_partition.current[0].partition}:logs:${data.aws_regions.current[0].names[0]}:${data.aws_caller_identity.current[0].account_id}:log-group:${log_group.name}:*"
  ]
}
